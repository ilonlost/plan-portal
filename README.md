...
Подготовлены три представления: временные шкалы ПЦ/КЦ по местам затрат, сводка по артикулам и дням, схема этапов цеха. Период ограничен 184 днями. Есть детализация и Excel-отчёт.

**Данные этого раздела — явно обозначенная заглушка `erp_stub`, а не реальный факт CSB/ERP.** Подключение таблицы простоев не подключает производственные проводки. По умолчанию раздел доступен администратору; другому пользователю его можно открыть индивидуально.
...
| `/api/production-fact` | Демонстрационные шкалы, детализация, сводки и экспорт |
...
Раздел **Факт производства → Шкала ПЦ / КЦ → Хвостовые буферы** читает реальные перемещения отдельно от демонстрационных шкал. Настройка на VPS в `/opt/plan-portal/.env`:
...
Первое движение SSCC определяется по минимальному `L52_REC_NR` **в доступных месячных таблицах выбранного периода**. Это не обещание первого движения за всю историю. Даты проводок без часового пояса трактуются как московские. Поля количества и достоверных часов производства в предоставленном SQL отсутствуют: отображаются число проводок и уникальных SSCC, без выдуманных кг/коробов/часов.

API: `GET /api/production-fact/tail-buffers?start=YYYY-MM-DD&end=YYYY-MM-DD`; доступ наследуется от раздела факта. Период до 184 дней, таймаут чтения 30 секунд, максимум 5000 последних проводок; при превышении показываются общее количество и предупреждение об усечении. Таблица интерфейса имеет страницы по 100 строк и общий вертикальный скролл. Недоступные месяцы отображаются явно, данные помечаются частичными. Подключение выполняет только SELECT и не меняет план, справочник или DWH.

Без настроенного подключения все 12 МЗ отображаются с состоянием ожидания, а не с тестовыми проводками. Проверка на реальной БД возможна после внесения рабочих реквизитов.
...
- Факт производства остаётся демонстрационным; автоматическая загрузка ERP-факта в АЗ и отправка в CSB ещё не реализованы.
