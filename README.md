...
Раздел полностью использует реальные проводки хвостовых буферов CSB DWH. Две шкалы ПЦ/КЦ показывают активность всех 12 МЗ: число проводок в интервалах 15 минут за день, по часам за 2–7 дней и по дням за более длинный период. Нажатие на МЗ открывает его детализацию. Разделы «Артикулы ГП» и «Карта цехов» скрыты, генератор демонстрационных данных удалён.
...
Период ограничен 184 днями. Проводки не определяют время работы, простои или массу выпуска — таких расчётных показателей здесь нет. По умолчанию раздел доступен администратору; другому пользователю его можно открыть индивидуально.
...
- Факт производства содержит реальные проводки хвостовых буферов. Для часов работы, пауз, кг и карты этапов нужны дополнительные поля/источники. Автоматическая загрузка ERP-факта в АЗ и отправка в CSB ещё не реализованы.
