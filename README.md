...
В файле CSB количество `L1+` и время `T55+` передаются целыми числами с округлением вверх (`3.6` → `4`, `3.0` → `3`). Время по умолчанию берётся из расчётной длительности задания в часах; явно заданное числовое значение «T55 CSB» в настройке линии имеет приоритет и также округляется. Исходные количество и длительность в плане не меняются. Некорректное значение T55 блокирует формирование файла с понятной ошибкой, без смены статусов заданий. После успешного формирования TXT включённые задания со статусом «Не начато» получают статус «Выгружено»; статусы исполнения «В работе», «Выполнено» и отгрузки сохраняются. Мойки/ТО, исключённые задания, задания вне выбранных дат и без кода CSB не помечаются. Статус означает подготовку файла, а не подтверждение его приёма внешней CSB. Выгрузка и список ID записываются в журнал интеграций и аудит; повторное скачивание разрешено.
...
В таблице проводок и на листе XLSX «Проводки» есть «Срок годности, суток»: текущее значение `ART_CARD.SY0012_HBK_ZEIT` из `cp_DWH_SY0012_SY8212_SY9014_SY9118`, сопоставленное по `SY0012_NR` с артикулом карточки SSCC. Срок хранится в отчёте числом. Отсутствующее/некорректное значение, неоднозначный артикул SSCC или несколько разных сроков одного артикула показываются пустыми (на сайте — «—»). Нулевое значение сохраняется как 0; данные о сроке не изменяют вес, циклы и число проводок.
...
### Производительность и фоновое обновление

Расчёт плана читает мощности только в пределах его периода. Пересчёт загрузки учитывает фактические даты заданий, включая ручные задания за границами периода. Исторические мощности сохраняются в БД, но не загружаются целиком в память при каждом изменении справочника или задания. Сводки линий и мощностей используют групповые SQL-запросы вместо отдельного запроса на каждую строку.

В браузере каждые 10 секунд после завершения предыдущей проверки проверяются права пользователя и, только на странице плана, `GET /api/plans/active/revision` (`id`, `version`). Полная матрица загружается при изменении версии/активного плана, фильтров или после собственной правки. Справочник загружается только в разделах «Справочник» и «Источники данных». Неизменившиеся ответы не вызывают повторную перерисовку; фоновые проверки приостанавливаются в скрытой вкладке, устаревшие запросы отменяются при смене раздела/периода. Простои проверяются раз в 30 секунд после завершения предыдущего запроса. Nginx сжимает JSON, JS и CSS через gzip.

Для диагностики сравнивайте время SQL/пересчёта с временем передачи ответа и отображения в браузере. Проверка оптимизации от 24.09.2026 на копии расчёта в откатываемой транзакции VPS: 14,559 → 1,914 секунды; все 1 338 результирующих заданий совпали. Это замер конкретного плана, а не гарантия времени для любого объёма. Регрессионные проверки находятся в `backend/tests/test_performance_regressions.py`.
