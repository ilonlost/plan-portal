...
export default function App() {
  ...
  const selectDate = (value: string) => setViewDate(viewDays === 7 ? startOfWeek(value) : value);
  const selectDays = (value: ViewDays) => { setViewDays(value); if (value === 7) setViewDate(startOfWeek(viewDate)); };
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="sidebar-tip"><span>?</span><b>Нужна помощь?</b><small>Инструкция, роли и выгрузки</small><button onClick={() => setHelpOpen(true)}>Открыть инструкцию</button><button onClick={() => setPage("feedback")}>Написать в ИТ</button></div>
    ...
    <main>
      ...
      <div className="content">{loading ? <Loading /> : <>
        {page === "plan" && <div className="plan-scale" style={{ "--plan-zoom": String(planZoom / 100) } as React.CSSProperties}><PlanView matrix={matrix} workshops={workshops} lines={lines} user={user}
          ...
          onMoved={async (item, targetDate) => { try { await api.updateItem(matrix!.plan.id, item.id, { production_date: targetDate, line_id: item.line_id, locked: true, comment: "Перенос карточки в недельном плане" }, matrix!.plan.version); setNotice(`Задание №${item.sequence} перенесено на ${formatDate(targetDate)}`); await refreshPlan(); } catch (reason) { setError(message(reason)); } }}
...
function PlanView({ matrix, workshops, lines, user, selectedWorkshop, selectedLine, viewDate, viewDays, dayLayout, onWorkshop, onLine, onReset, onDate, onDays, onPrevious, onNext, onLayout, onMoved, onItem, onUpload, onCreate }: {
  ...
  return <div className="stack">
    {(matrix.unscheduled || []).length > 0 && <details className="card unplaced-list"><summary>Невозможно проработать / разместить такой объем · {matrix.unscheduled.length} позиций</summary><p>Эти задания не занимают мощность. Исправьте справочник или увеличьте доступную мощность.</p><div className="unplaced-items">{matrix.unscheduled.map(item => <button className="button secondary" key={item.id} onClick={() => onItem(item)}>{item.sku} · {number(item.quantity_kg || 0)} кг · {item.warnings.join("; ")}</button>)}</div></details>}
...
function ItemDrawer({ item, planId, planVersion, user, lines, onClose, onChanged, onError }: { item: ScheduleItem; planId: number; planVersion: number; user: UserProfile; lines: LineData[]; onClose: () => void; onChanged: (t: string) => Promise<void>; onError: (t: string) => void }) {
  ...
  const remove = async () => {
    if (!window.confirm(`Удалить задание ${item.sku}? Оно будет исключено из последующих пересчётов.`)) return;
    setBusy(true);
...
function CatalogView({ data, lines, user, onSaved }: { data: CatalogData; lines: LineData[]; user: UserProfile; onSaved: (t: string) => Promise<void> }) {
  const [section, setSection] = useState<"products" | "parameters" | "lines" | "capacity">("products");
  ...
  const rows = useMemo(() => data.rows.filter(row => (!workshop || row.workshop_code === workshop) && (!query || `${row.sku} ${row.product_name} ${row.line_name}`.toLowerCase().includes(query.toLowerCase()))), [data.rows, query, workshop]);
  const products = useMemo(() => data.products.filter(product => !query || `${product.sku} ${product.product_name} ${product.line_names.join(" ")}`.toLowerCase().includes(query.toLowerCase())), [data.products, query]);
  ...
  const nav = [["products", "Артикулы", data.products.length], ["parameters", "Параметры SKU", data.rows.length], ["lines", "Линии и CSB", lines.length], ["capacity", "Графики и мощности", lines.length]] as const;
  return <div className="stack">
    ...
    <div className="section-shell"><aside className="section-nav card"><small>СПРАВОЧНИК</small>{nav.map(([value, label, count]) => <button key={value} className={section === value ? "active" : ""} onClick={() => { setSection(value); setQuery(""); }}><span>{label}</span><em>{count}</em></button>)}</aside><div className="section-content">
      {section === "products" && <section className="card catalog-card"><div className="catalog-head"><div><h2>Артикулы</h2><p>Нажмите на строку, чтобы изменить артикул, его линию или состояние.</p></div><div className="filters"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Артикул, продукция или линия…" />{canPlan(user) && <button className="button primary" onClick={() => setEditingProduct("new")}>Создать артикул</button>}</div></div><div className="table-scroll integrated-table-scroll"><table><thead><tr><th>Артикул / продукция</th><th>Линия</th><th>Тип</th><th>Статус даты</th><th>Статус арт</th><th>Упаковка</th></tr></thead><tbody>{products.map(product => <tr key={product.product_id} className={`${canPlan(user) ? "clickable-row" : ""} ${product.catalog_status === "blocked" ? "blocked-row" : ""}`} onClick={() => canPlan(user) && setEditingProduct(product)}><td><b>{product.sku}</b><small>{product.product_name}</small>{product.catalog_status === "blocked" && <span className="status-chip warning">Заблокирован</span>}</td><td>{product.line_names.length ? product.line_names.join(", ") : <span className="bad">Линия не назначена</span>}</td><td><span className={`source ${productType(product.state) === "ЗАМ" ? "zam" : "ohl"}`}>{productType(product.state)}</span></td><td>{product.advance_status || "—"}</td><td>{product.fk_status || "—"}</td><td>{product.units_per_box ? `${number(product.units_per_box)} шт. · ${product.box_weight_kg ? `${number(product.box_weight_kg)} кг` : "вес не задан"}` : "—"}</td></tr>)}</tbody></table></div><footer>Показано {products.length} из {data.products.length}</footer></section>}
      {section === "parameters" && <section className="card catalog-card"><div className="catalog-head"><div><h2>Параметры SKU</h2><p>Нажмите на строку для редактирования производственных параметров.</p></div><div className="filters"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Артикул, продукция, линия…" /><select value={workshop} onChange={e => setWorkshop(e.target.value)}><option value="">Все цеха</option><option value="PC">ПЦ</option><option value="KC">КЦ</option></select></div></div><div className="table-scroll integrated-table-scroll"><table><thead><tr><th>Артикул / продукция</th><th>Цех / линия</th><th>Скорость</th><th>Квант замеса, кг</th><th>Короб</th><th>Монопродукт</th><th>Рецептура</th></tr></thead><tbody>{rows.map(row => <tr key={row.capability_id} className={canPlan(user) ? "clickable-row" : ""} onClick={() => canPlan(user) && setEditing(row)}><td><b>{row.sku}</b><small>{row.product_name}</small></td><td><span className="workshop-code">{row.workshop_name}</span><small>{row.line_name}</small></td><td><strong>{number(row.speed_kg_hour)} кг/ч</strong></td><td>{row.batch_quantum_kg ? number(row.batch_quantum_kg) : "—"}</td><td>{row.units_per_box ? `${number(row.units_per_box)} шт.` : "—"}</td><td>{row.workshop_code === "PC" ? row.mono_group || "Авто" : "—"}</td><td><button className="button secondary recipe-button" onClick={event => { event.stopPropagation(); setBomProduct({ id: row.product_id, sku: row.sku }); }}>Получить спецификацию</button></td></tr>)}</tbody></table></div><footer>Показано {rows.length} из {data.rows.length}</footer></section>}
      {section === "lines" && <CatalogLinesTable lines={lines} user={user} onSelect={setScheduleLine} title="Производственные линии и коды CSB" description="Рабочие линии из утверждённой структуры ФК." />}
      {section === "capacity" && <CatalogLinesTable lines={lines} user={user} onSelect={setScheduleLine} title="Графики и мощности" description="График, смены, доступная мощность и код CSB настраиваются в едином редакторе линии." />}
    </div></div>
    ...
    {editing && <CatalogEditor row={editing} lines={lines} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await onSaved("Параметры SKU обновлены"); }} />}
...
function CatalogLinesTable({ lines, user, onSelect, title, description }: { lines: LineData[]; user: UserProfile; onSelect: (id: number) => void; title: string; description: string }) {
  return <section className="card catalog-card"><div className="catalog-head"><div><h2>{title}</h2><p>{description}</p></div></div><div className="table-scroll integrated-table-scroll"><table><thead><tr><th>Цех</th><th>Линия</th><th>График</th><th>Код CSB</th><th>Артикулы</th>{canPlan(user) && <th />}</tr></thead><tbody>{lines.map(line => <tr key={line.id}><td><span className="workshop-code">{line.workshop_name}</span></td><td><b>{line.name}</b><small>{line.code}</small></td><td>{line.schedule_label}<small>{number(line.working_hours)} ч/сутки</small></td><td><strong>{line.csb_line_code || <span className="bad">Не задан</span>}</strong></td><td>{number(line.product_count)}</td>{canPlan(user) && <td><button className="button secondary" onClick={() => onSelect(line.id)}>Настроить</button></td>}</tr>)}</tbody></table></div></section>;
}
...
function ProductEditor({ product, lines, onClose, onSaved }: { product: CatalogProduct | null; lines: LineData[]; onClose: () => void; onSaved: (text: string) => Promise<void> }) {
  ...
  const save = async () => { setBusy(true); setError(""); try { if (product) await api.updateProduct(product.product_id, payload()); else await api.createProduct(payload()); await onSaved(product ? "Артикул и параметры SKU обновлены" : "Артикул создан"); } catch (e) { setError(message(e)); } finally { setBusy(false); } };
  const lifecycle = async (status: "active" | "blocked") => { if (!product) return; setBusy(true); try { await api.updateProductStatus(product.product_id, status); await onSaved(status === "blocked" ? "Артикул заблокирован; план сохранён" : "Артикул разблокирован"); } catch (e) { setError(message(e)); setBusy(false); } };
  const remove = async () => { if (!product) return; if (!confirmDelete) { setConfirmDelete(true); return; } setBusy(true); try { await api.deleteProduct(product.product_id); await onSaved("Артикул скрыт из справочника; существующий план сохранён"); } catch (e) { setError(message(e)); setBusy(false); } };
...
function BomModal({ product, onClose }: { product: { id: number; sku: string }; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="modal bom-modal"><header><div><small>СПЕЦИФИКАЦИЯ · {product.sku}</small><h2>{data?.product_name || "Загрузка спецификации…"}</h2><p>Данные запрашиваются из корпоративной системы BOM.</p></div><button onClick={onClose}>×</button></header>{error && <div className="inline-error">{error}</div>}{data && <><div className="bom-basis">Расчёт спецификации на <b>{number(data.basis_units)} шт.</b></div>{data.rows.length ? <div className="table-scroll integrated-table-scroll"><table><thead><tr>{data.columns.map(column => <th key={column}>{column}</th>)}</tr></thead><tbody>{data.rows.map((row, index) => <tr key={index}>{data.columns.map(column => <td key={column}>{String(row[column] ?? "—")}</td>)}</tr>)}</tbody></table></div> : <div className="empty">Для артикула нет строк спецификации</div>}</>}<footer><button className="button secondary" onClick={onClose}>Закрыть</button></footer></section></div>;
}
...
function LineScheduleModal({ lines, initialLineId, onClose, onSaved }: { lines: LineData[]; initialLineId?: number; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  const save = async () => { if (!data) return; setBusy(true); setError(""); try { await api.updateLineSchedule(lineId, { schedule_code: data.schedule_code, template_id: data.template_id, anchor_date: data.anchor_date, mail_recipients: data.mail_recipients, csb_line_code: data.csb_line_code, csb_t5: data.csb_t5, csb_t55: data.csb_t55, slots: mode === "manual" ? data.slots.filter(slot => dirty.has(slot.capacity_date)).map(slot => ({ capacity_date: slot.capacity_date, day_hours: slot.day_hours, night_hours: slot.night_hours, note: "Ручная корректировка" })) : [] }); await onSaved(); } catch (e) { setError(message(e)); } finally { setBusy(false); } };
  ...
  return <div className="modal-backdrop"><div className="modal schedule-modal"><header><div><small>ДОСТУПНЫЕ ЧАСЫ И СМЕНЫ</small><h2>График работы линий</h2><p>Шаблоны и разовые ручные изменения настраиваются отдельно.</p></div><button onClick={onClose}>×</button></header>{error && <div className="inline-error">{error}</div>}{notice && <div className="inline-success">{notice}</div>}
    ...
    <footer><button className="button secondary" onClick={onClose}>Отмена</button>{mode !== "create" && <button className="button primary" disabled={busy || !data || (mode === "manual" && dirty.size === 0)} onClick={() => void save()}>{mode === "apply" ? "Применить шаблон и пересчитать" : "Сохранить ручные изменения"}</button>}</footer></div></div>;
}
...
function AdminView({ onDeleted, onError }: { onDeleted: (t: string) => Promise<void>; onError: (t: string) => void }) {
  ...
  const sections = [["overview", "Обзор"], ["access", "Пользователи и права"], ["mail", "Письма"], ["audit", "Аудит"]] as const;
...
function AdminUserRow({ row, onSaved, onError }: { row: AdminOverview["users"][number]; onSaved: () => Promise<void>; onError: (v: string) => void }) {
  const [role, setRole] = useState(portalRoles.some(item => item.value === row.role) ? row.role : "viewer"); const [active, setActive] = useState(row.active); const [busy, setBusy] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false);
  ...
  const remove = async () => { if (!confirmDelete) { setConfirmDelete(true); return; } setBusy(true); try { await api.deleteUserAccess(row.id); await onSaved(); } catch (e) { onError(message(e)); setBusy(false); setConfirmDelete(false); } };
  return <tr><td><b>{row.display_name}</b><small>{row.username}{row.email ? ` · ${row.email}` : ""}</small>{row.role === "master" && <small className="legacy-role">Назначьте одну из актуальных ролей</small>}</td><td><select value={role} onChange={e => setRole(e.target.value)}>{portalRoles.map(item => <option value={item.value} key={item.value}>{item.label}</option>)}</select></td><td><label className="check-label"><input type="checkbox" checked={active} onChange={e => setActive(e.target.checked)} /> Активен</label></td><td><div className="inline-actions"><button className="button secondary" disabled={busy} onClick={() => void save()}>Сохранить</button><button className="button danger" disabled={busy} onClick={() => void remove()}>{confirmDelete ? "Подтвердить" : "Удалить"}</button></div></td></tr>;
}
...
function PlanMailModal({ planId, start, end, lines, onClose, onNotice, onError }: { planId: number; start: string; end: string; lines: LineData[]; onClose: () => void; onNotice: (v: string) => void; onError: (v: string) => void }) {
  const [audience, setAudience] = useState("production");
  ...
  const preview = async () => { setBusy(true); try { const result = await api.mailPreview(from, to, lineIds, audience); setHtml(result.html); setCount(result.item_count); } catch (e) { onError(message(e)); } finally { setBusy(false); } };
  useEffect(() => { void preview(); }, [audience]);
  const send = async () => { setBusy(true); try { const result = await api.emailPlan(planId, recipients.split(/[,;\n]/).map(v => v.trim()).filter(Boolean), from, to, lineIds, audience); onNotice(result.status === "sent" ? `План отправлен: ${result.recipients.join(", ")}` : `Письмо сформировано, отправка: ${result.error || result.status}`); onClose(); } catch (e) { onError(message(e)); } finally { setBusy(false); } };
