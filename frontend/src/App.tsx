...
function LineScheduleModal({ lines, initialLineId, onClose, onSaved }: { lines: LineData[]; initialLineId?: number; onClose: () => void; onSaved: () => Promise<void> }) {
  const [lineId, setLineId] = useState(initialLineId || lines[0]?.id || 0); const [start, setStart] = useState(today()); const [data, setData] = useState<LineScheduleData | null>(null); const [dirty, setDirty] = useState<Set<string>>(new Set()); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [templateName, setTemplateName] = useState("");
