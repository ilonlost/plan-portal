...
function CatalogView({ data, lines, user, onSaved }: { data: CatalogData; lines: LineData[]; user: UserProfile; onSaved: (t: string) => Promise<void> }) {
  const [query, setQuery] = useState(""); const [workshop, setWorkshop] = useState(""); const [editing, setEditing] = useState<CatalogRow | null>(null); const [scheduleLine, setScheduleLine] = useState<number | null>(null); const [section, setSection] = useState<"products" | "parameters" | "lines" | "capacity">("products");
  ...
  const nav = [
    ["products", "Артикулы", `${data.products.length}`], ["parameters", "Параметры SKU", `${data.rows.length}`],
    ["lines", "Линии и CSB", `${lines.length}`], ["capacity", "Графики и мощности", ""],
  ] as const;
  return <div className="stack"><section className="metric-grid"><Metric label="Артикулов" value={number(data.summary.products)} note={`${data.unmapped_products.length} без линии`} tone="red" /><Metric label="Связей линия–SKU" value={number(data.summary.capabilities)} note="скорость и квант" tone="blue" /><Metric label="Линий" value={number(data.summary.lines)} note="только рабочие линии" tone="green" /><Metric label="SKU с рецептурой" value={number(data.summary.with_recipes)} note="компоненты сырья" tone="amber" /></section><div className="section-shell"><aside className="section-nav card"><small>СПРАВОЧНИК</small>{nav.map(([value, label, count]) => <button key={value} className={section === value ? "active" : ""} onClick={() => { setSection(value); setQuery(""); }}><span>{label}</span>{count && <em>{count}</em>}</button>)}</aside><div className="section-content">
    {section === "products" && <div className="stack"><UnmappedProducts data={data} lines={lines} user={user} onSaved={onSaved} /><section className="card catalog-card"><div className="catalog-head"><div><h2>Все артикулы</h2><p>Включая позиции из ОХЛ и ЗАМ, для которых ещё не задана производственная линия.</p></div><div className="filters"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Артикул, название или линия…" /></div></div><div className="table-scroll integrated-table-scroll"><table><thead><tr><th>Артикул / продукция</th><th>Линии</th><th>Статус даты</th><th>Статус ФК</th><th>Упаковка</th></tr></thead><tbody>{products.map(product => <tr key={product.product_id}><td><b>{product.sku}</b><small>{product.product_name}</small></td><td>{product.line_names.length ? product.line_names.join(", ") : <span className="bad">Линия не назначена</span>}</td><td>{product.advance_status || "—"}</td><td>{product.fk_status || "—"}</td><td>{product.units_per_box ? `${number(product.units_per_box)} шт. в коробе` : "—"}</td></tr>)}</tbody></table></div><footer>Показано {products.length} из {data.products.length}</footer></section></div>}
    {section === "parameters" && <section className="card catalog-card"><div className="catalog-head"><div><h2>Параметры производства SKU</h2><p>Скорость, замес, упаковка и технологические ограничения для связи артикула с линией.</p></div><div className="filters"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Артикул, название, линия…" /><select value={workshop} onChange={e => setWorkshop(e.target.value)}><option value="">Все цеха</option><option value="PC">ПЦ</option><option value="KC">КЦ</option></select></div></div><div className="table-scroll"><table><thead><tr><th>Артикул / продукция</th><th>Цех / линия</th><th>Скорость</th><th>Квант замеса</th><th>Монопродукт</th><th>Короб</th><th>Старый план</th><th>Рецептура</th>{canPlan(user) && <th />}</tr></thead><tbody>{rows.map(row => <tr key={row.capability_id}><td><b>{row.sku}</b><small>{row.product_name}</small><small>{row.advance_status || "Авансовая дата не задана"} · {row.fk_status || "Статус ФК не задан"}</small></td><td><span className="workshop-code">{row.workshop_name}</span><small>{row.line_name}</small></td><td><strong>{number(row.speed_kg_hour)} кг/ч</strong></td><td>{row.batch_quantum_kg ? `${number(row.batch_quantum_kg)} кг` : "—"}</td><td>{row.workshop_code === "PC" ? row.mono_group || "Авто по названию" : "—"}</td><td>{row.units_per_box ? `${number(row.units_per_box)} шт.` : "—"}</td><td>{row.legacy_quantum_units ? `${number(row.legacy_quantum_units)} ${row.legacy_capacity_unit}` : "—"}</td><td>{row.recipe_component_count ? `${row.recipe_component_count} комп.` : "—"}</td>{canPlan(user) && <td><button className="icon-button" aria-label={`Изменить ${row.sku}`} onClick={() => setEditing(row)}>✎</button></td>}</tr>)}</tbody></table></div><footer>Показано {rows.length} из {data.rows.length}</footer></section>}
    {section === "lines" && <section className="card catalog-card"><div className="catalog-head"><div><h2>Производственные линии и коды CSB</h2><p>В списке остаются только рабочие линии из утверждённой структуры ФК.</p></div></div><div className="table-scroll integrated-table-scroll"><table><thead><tr><th>Цех</th><th>Линия</th><th>Код CSB</th><th>Режим</th><th>Артикулы</th>{canPlan(user) && <th />}</tr></thead><tbody>{lines.map(line => <tr key={line.id}><td><span className="workshop-code">{line.workshop_name}</span></td><td><b>{line.name}</b><small>{line.code}</small></td><td><strong>{line.csb_line_code || <span className="bad">Не задан</span>}</strong></td><td>{line.schedule_label}</td><td>{number(line.product_count)}</td>{canPlan(user) && <td><button className="button secondary" onClick={() => setScheduleLine(line.id)}>Настроить</button></td>}</tr>)}</tbody></table></div></section>}
    {section === "capacity" && <section className="card capacity-page"><header><small>ГРАФИКИ ЛИНИЙ</small><h2>Смены и доступная мощность</h2><p>Выберите линию. В одном редакторе задаются график, часы смен, код CSB и получатели.</p></header><div className="line-settings-grid">{lines.map(line => <button key={line.id} onClick={() => setScheduleLine(line.id)}><span className="workshop-code">{line.workshop_name}</span><b>{line.name}</b><small>{line.schedule_label} · CSB {line.csb_line_code || "не задан"}</small></button>)}</div></section>}
  </div></div>{editing && <CatalogEditor row={editing} lines={lines} onClose={() => setEditing(null)} onSaved={async () => { setEditing(null); await onSaved("Параметры линии обновлены"); }} />}{scheduleLine != null && <LineScheduleModal lines={lines} initialLineId={scheduleLine} onClose={() => setScheduleLine(null)} onSaved={async () => { setScheduleLine(null); await onSaved("График сохранён, производственный план пересчитан"); }} />}</div>;
}

function CatalogEditor({ row, lines, onClose, onSaved }: { row: CatalogRow; lines: LineData[]; onClose: () => void; onSaved: () => Promise<void> }) {
  const [selectedLine, setSelectedLine] = useState(row.line_id);
  const [lineStatus, setLineStatus] = useState(row.line_status || "");
  const [state, setState] = useState(row.state || "");
  const [category, setCategory] = useState(row.category || "");
  const [advance, setAdvance] = useState(row.advance_status || "");
  const [fkStatus, setFkStatus] = useState(row.fk_status || "");
  const [productName, setProductName] = useState(row.product_name);
  const [weight, setWeight] = useState(String(row.unit_weight_kg || ""));
  const [boxWeight, setBoxWeight] = useState(String(row.box_weight_kg || ""));
  const [boxUnits, setBoxUnits] = useState(String(row.units_per_box || ""));
  ...
  const save = async () => { try { await api.updateCapability(row.capability_id, { line_id: selectedLine, line_status: lineStatus, state, category, advance_status: advance || undefined, fk_status: fkStatus, product_name: productName, unit_weight_kg: weight ? Number(weight) : null, box_weight_kg: boxWeight ? Number(boxWeight) : null, units_per_box: boxUnits ? Number(boxUnits) : null, units_per_hour: Number(speed), batch_quantum_kg: quantum ? Number(quantum) : null, min_order_kg: minimum ? Number(minimum) : null, restrictions, mono_group: mono || null }); await onSaved(); } catch (e) { setError(message(e)); } };
  return <div className="modal-backdrop"><div className="modal"><header><div><small>{row.sku} · {row.line_name}</small><h2>Параметры линии</h2></div><button onClick={onClose}>×</button></header>{error && <div className="inline-error">{error}</div>}<div className="form-grid"><label>Линия<select value={selectedLine} onChange={e => setSelectedLine(Number(e.target.value))}>{lines.map(line => <option key={line.id} value={line.id}>{line.workshop_name} · {line.name}</option>)}</select></label><label>Статус в файле мощностей<input value={lineStatus} onChange={e => setLineStatus(e.target.value)} /></label><label>Состояние продукции<input value={state} onChange={e => setState(e.target.value)} /></label><label>Категория<input value={category} onChange={e => setCategory(e.target.value)} /></label><label className="full">Наименование<input value={productName} onChange={e => setProductName(e.target.value)} /></label><label>Авансовая дата<select value={advance} onChange={e => setAdvance(e.target.value)}><option value="">Не задано</option><option>АЗ</option><option>По графику</option></select></label><label>Статус Справочника ФК<input value={fkStatus} onChange={e => setFkStatus(e.target.value)} /></label><label>Вес единицы, кг<input type="number" value={weight} onChange={e => setWeight(e.target.value)} /></label><label>Вес короба, кг<input type="number" value={boxWeight} onChange={e => setBoxWeight(e.target.value)} /></label><label>Штук в коробе<input type="number" value={boxUnits} onChange={e => setBoxUnits(e.target.value)} /></label><label>Скорость, кг/ч<input type="number" value={speed} onChange={e => setSpeed(e.target.value)} /></label><label>Квант замеса, кг<input type="number" value={quantum} onChange={e => setQuantum(e.target.value)} /></label><label>Минимальный заказ, кг<input type="number" value={minimum} onChange={e => setMinimum(e.target.value)} /></label>{row.workshop_code === "PC" && <label>Группа монопродукта<input value={mono} onChange={e => setMono(e.target.value)} placeholder="Пусто — определить автоматически" /></label>}<label className="full">Ограничения<textarea value={restrictions} onChange={e => setRestrictions(e.target.value)} /></label></div><footer><button className="button secondary" onClick={onClose}>Отмена</button><button className="button primary" onClick={() => void save()}>Сохранить</button></footer></div></div>;
}
