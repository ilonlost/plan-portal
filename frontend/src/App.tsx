...
function Login({ mode, error, onLogin }: { mode: SessionMode | null; error: string | null; onLogin: (u: string, p: string) => Promise<void> }) {
  ...
  return <div className="login-page"><div className="login-brand"><span>PP</span><b>PLAN PORTAL</b></div><form className="login-card" onSubmit={submit}>
    ...
    <footer>{mode?.auth_mode === "mock" ? "Локальное окружение разработки" : "Авторизация через корпоративный LDAP"}</footer>
  </form></div>;
}
...
function ItemDrawer({ item, planId, planVersion, user, lines, onClose, onChanged, onError }: { item: ScheduleItem; planId: number; planVersion: number; user: UserProfile; lines: LineData[]; onClose: () => void; onChanged: (t: string) => Promise<void>; onError: (t: string) => void }) {
  ...
  const savedDraft = useRef(JSON.stringify({ date: item.production_date || "", shift: item.shift, quantity: String(item.quantity_kg || item.quantity), lineId: String(item.line_id || "") }));
  const [saveState, setSaveState] = useState("");
  const [status, setStatus] = useState(item.execution_status); const [note, setNote] = useState(item.execution_note || "");
  const [date, setDate] = useState(initialDraft?.date || item.production_date || ""); const [shift, setShift] = useState(initialDraft?.shift || item.shift);
  ...
  useEffect(() => {
    const draft = JSON.stringify({ date, shift, quantity, lineId });
    ...
    const timer = window.setTimeout(async () => {
      ...
      setBusy(true);
      try {
        const result = await api.updateItem(planId, item.id, { production_date: date, shift, quantity: Number(quantity), line_id: Number(lineId), locked: true, comment: "Автосохранение" }, editVersion);
  ...
  }, [date, shift, quantity, lineId, busy, editVersion]);
  const compare = async () => {
