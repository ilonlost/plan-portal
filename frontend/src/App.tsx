    ...
    <main>
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          {page === "plan" && matrix && canPlan(user) && <a className="button secondary" href={api.csbDownloadUrl()}>Скачать файл для прогрузки</a>}
...
function ManualTaskModal({ planId, lines, initialDate, onClose, onSaved, onError }: { planId: number; lines: LineData[]; initialDate: string; onClose: () => void; onSaved: () => Promise<void>; onError: (v: string) => void }) {
  const [lineId, setLineId] = useState(lines[0]?.id || 0); const [products, setProducts] = useState<{ product_id: number; sku: string; name: string; speed_kg_hour: number }[]>([]); const [productId, setProductId] = useState(""); const [productionDate, setProductionDate] = useState(initialDate); const [markingDate, setMarkingDate] = useState(initialDate); const [shift, setShift] = useState("day"); const [quantity, setQuantity] = useState(""); const [sourceKind, setSourceKind] = useState("generic"); const [busy, setBusy] = useState(false);
  useEffect(() => { if (!lineId) return; api.manualProducts(lineId).then(rows => { setProducts(rows); setProductId(rows[0] ? String(rows[0].product_id) : ""); }).catch(e => onError(message(e))); }, [lineId]);
