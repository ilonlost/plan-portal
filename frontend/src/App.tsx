...
import type {
  ...
  MailConfiguration, MatrixData, ScheduleItem, SessionMode, UserProfile, WorkshopData,
} from "./types";
import "./styles.css";
...
function PlanView({ matrix, workshops, lines, user, selectedWorkshop, selectedLine, viewDate, viewDays, dayLayout, onWorkshop, onLine, onReset, onDate, onDays, onPrevious, onNext, onLayout, onItem, onUpload }: {
  ...
  return <div className="stack">
    ...
    {dayLayout === "table" ? <PeriodTable matrix={matrix} onItem={onItem} /> : viewDays === 1 ? <DailyBoard matrix={matrix} onLine={onLine} onItem={onItem} /> : <PlanMatrix matrix={matrix} onLine={onLine} onItem={onItem} />}
  </div>;
}
...
function PlanMatrix({ matrix, onLine, onItem }: { matrix: MatrixData; onLine: (w: string, id: number) => void; onItem: (i: ScheduleItem) => void }) {
  return <section className="matrix-card card">
    <header className="matrix-toolbar"><div><h3>План по линиям и дням</h3><p>Артикулы, штуки, килограммы и часы видны прямо в ячейках.</p></div><div className="legend"><span><i className="dot ohl" />ОХЛ</span><span><i className="dot zam" />ЗАМ</span></div></header>
    <div className="matrix-scroll"><div className="matrix" style={{ gridTemplateColumns: `220px repeat(${matrix.dates.length}, 220px)` }}>
      ...
      {matrix.workshops.map(workshop => <div className="workshop-fragment" key={workshop.code}>
        ...
        {workshop.lines.map(line => <div className="line-fragment" key={line.id}>
          ...
          {line.cells.map(cell => <div key={cell.date} className={`plan-cell ${cellTone(cell)}`}>
            <div className="cell-load"><b>{cell.load_percent.toFixed(0)}%</b><small>{number(cell.planned_hours)}/{number(cell.capacity_hours)} ч</small></div><div className="cell-bar"><i style={{ width: `${Math.min(100, cell.load_percent)}%` }} /></div>
            <div className="cell-skus">{cell.items.slice(0, 5).map(item => <button key={item.id} onClick={() => onItem(item)}><span className={`source ${item.source_kind}`}>{sourceLabels[item.source_kind]}</span><b>{item.sku}</b><small title={item.product_name}>{item.product_name}</small><em>{item.quantity_units == null ? "—" : `${number(item.quantity_units)} шт.`} · {number(item.quantity_kg || 0)} кг · {item.shift === "day" ? "д" : "н"}</em></button>)}{cell.items.length > 5 && <strong>+ ещё {cell.items.length - 5}</strong>}{!cell.items.length && <div className="empty-capacity">{number(cell.gap_hours)} ч свободно</div>}</div>
          </div>)}
        </div>)}
      </div>)}
    </div></div>
...
function PeriodTable({ matrix, onItem }: { matrix: MatrixData; onItem: (i: ScheduleItem) => void }) {
  ...
  matrix.dates.forEach(day => {
    ...
    if (!dayCount) return;
    total += dayCount;
    rows.push(<tr className="date-group-row" key={`date-${day}`}><td colSpan={13}><b>{formatDate(day)}</b><span>{weekday(day)} · {weekNumber(day)} неделя · {dayCount} позиций</span></td></tr>);
    matrix.workshops.forEach(workshop => {
      ...
      rows.push(<tr className="workshop-group-row" key={`workshop-${day}-${workshop.code}`}><td colSpan={13}><b>{workshop.code} · {workshop.name}</b><span>{workshopCount} позиций</span></td></tr>);
      workshop.lines.forEach(line => {
        const cell = line.cells.find(value => value.date === day);
        if (!cell?.items.length) return;
        rows.push(<tr className="line-group-row" key={`line-${day}-${line.id}`}><td colSpan={13}><div className="line-group-content"><div><b>{line.name}</b><small>{line.code}</small></div><span><strong>{cell.load_percent.toFixed(0)}%</strong> · {number(cell.planned_hours)} / {number(cell.capacity_hours)} ч</span></div></td></tr>);
        cell.items.forEach(item => rows.push(<tr className="clickable-row item-row" key={`item-${day}-${item.id}`} onClick={() => onItem(item)}><td><span className={`source ${item.source_kind}`}>{sourceLabels[item.source_kind]}</span></td><td><b>{item.sku}</b></td><td><b>{item.product_name}</b></td><td>{item.shift === "day" ? "День" : "Ночь"}</td><td><strong>{item.quantity_units == null ? "—" : number(item.quantity_units)}</strong></td><td><strong>{number(item.quantity_kg || 0)}</strong></td><td><strong className="hours-value">{number(item.required_hours)} ч</strong></td><td>{item.box_count == null ? "—" : number(item.box_count)}</td><td>{item.batch_count == null ? "—" : number(item.batch_count)}</td><td>{formatDate(item.production_date)}</td><td>{formatDate(item.marking_date)}</td><td>{cell.load_percent.toFixed(0)}%</td><td><span className={`execution-badge ${item.execution_status}`}>{executionLabels[item.execution_status]}</span></td></tr>));
      });
    });
  });
  return <section className="card day-table period-table"><header><div><h3>Табличный план ГП</h3><p>{formatDate(matrix.dates[0])} — {formatDate(matrix.dates.at(-1))} · {total} позиций · дата → цех → линия</p></div></header><div className="table-scroll"><table><thead><tr><th>Источник</th><th>SKU</th><th>Готовая продукция</th><th>Смена</th><th>Штуки</th><th>Кг</th><th>Часы</th><th>Короба</th><th>Замесы</th><th>ДП</th><th>ДМ</th><th>Загрузка</th><th>Статус</th></tr></thead><tbody>{rows}</tbody></table></div></section>;
}
